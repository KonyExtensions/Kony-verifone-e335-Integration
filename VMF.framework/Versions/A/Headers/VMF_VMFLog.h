 ...
 * 
 * Step 3:
 * Replace your NSLog statements with VMF_VMFLog statements according to the severity of the message.
 * 
 * NSLog(@"Fatal error, no dohickey found!"); -> VMFLogError(@"Fatal error, no dohickey found!");
 * 
 * VMF_VMFLog works exactly the same as NSLog.
 ...
 * as the VMFLogMessage class captures the individual flag that caused the log message to fire.
 * 
 * -- Levels --
 * 
 ...
 *         DBLog(@"sprocket: %@", [sprocket description])
 * }
 * 
 * -- Async --
 * 
...
#define VMFLogError(frmt, ...)   LOG_OBJC_MAYBE(LOG_ASYNC_ERROR,   ddLogLevel, LOG_FLAG_ERROR,   0, frmt, ##__VA_ARGS__)
#define VMFLogWarn(frmt, ...)    LOG_OBJC_MAYBE(LOG_ASYNC_WARN,    ddLogLevel, LOG_FLAG_WARN,    0, frmt, ##__VA_ARGS__)
#define VMFLogInfo(frmt, ...)    LOG_OBJC_MAYBE(LOG_ASYNC_INFO,    ddLogLevel, LOG_FLAG_INFO,    0, frmt, ##__VA_ARGS__)
#define VMFLogVerbose(frmt, ...) LOG_OBJC_MAYBE(LOG_ASYNC_VERBOSE, ddLogLevel, LOG_FLAG_VERBOSE, 0, frmt, ##__VA_ARGS__)



#define VMFLogCError(frmt, ...)   LOG_C_MAYBE(LOG_ASYNC_ERROR,   ddLogLevel, LOG_FLAG_ERROR,   0, frmt, ##__VA_ARGS__)
#define VMFLogCWarn(frmt, ...)    LOG_C_MAYBE(LOG_ASYNC_WARN,    ddLogLevel, LOG_FLAG_WARN,    0, frmt, ##__VA_ARGS__)
#define VMFLogCInfo(frmt, ...)    LOG_C_MAYBE(LOG_ASYNC_INFO,    ddLogLevel, LOG_FLAG_INFO,    0, frmt, ##__VA_ARGS__)
#define VMFLogCVerbose(frmt, ...) LOG_C_MAYBE(LOG_ASYNC_VERBOSE, ddLogLevel, LOG_FLAG_VERBOSE, 0, frmt, ##__VA_ARGS__)

/**
 * The THIS_FILE macro gives you an NSString of the file name.
 * For simplicity and clarity, the file name does not include the full path or file extension.
 * 
 * For example: VMFLogWarn(@"%@: Unable to find thingy", THIS_FILE) -> @"MyViewController: Unable to find thingy"
 **/
 ...
 * 
 * For example: VMFLogWarn(@"%@ - Requires non-nil strings") -> @"setMake:model: requires non-nil strings"
 * 
...
+ (void)addLogger:(id <VMFLogger>)logger;
+ (void)removeLogger:(id <VMFLogger>)logger;
...
@protocol VMFLogger <NSObject>
@required

- (void)logMessage:(VMFLogMessage *)logMessage;
 ...
 * 
 * This allows invocations of VMFLog's flushLog method to be propogated to loggers that need it.
 * 
 * Note that VMFLog's flushLog method is invoked automatically when the application quits,
...
@protocol VMFLogFormatter <NSObject>
@required
...
- (NSString *)formatLogMessage:(VMFLogMessage *)logMessage;
...
/**
 * The VMFLogMessage class encapsulates information about the log message.
 * If you write custom loggers or formatters, you will be dealing with objects of this class.
 **/

@interface VMFLogMessage : NSObject
{
...
/**
 * The VMFLogger protocol specifies that an optional formatter can be added to a logger.
...
@interface VMF_DDAbstractLogger : NSObject <VMFLogger>
{
	id <VMFLogFormatter> formatter;
